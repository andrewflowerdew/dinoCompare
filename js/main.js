...
function 	Dino()																{
				...
				console.log("Species", species);
  				...
  				this.region 													= species.where;
  				this.period 													= species.when;
  				this.fact 													= species.fact;

}		

function 	Human()																{

				this.humanHeightFeetText											= document.getElementById("formHeightFeetInput").value;
				this.humanHeightInchesText										= document.getElementById("formHeightInchesInput").value;
				this.humanFeetInFeetNumber										= parseInt(humanHeightFeetText,10);
				this.humanHeightInchesNumber 										= parseInt(humanHeightInchesText,10);
				this.totalHumanHeightInches 										= (humanFeetInFeetNumber*12)+humanHeightInchesNumber;
		...
		let dinoDb                          										= `../data/dino.json`;
            ...
            .then(function(response)                   								{
                 ...
                 return response.json();
              	  ...
              	  createDinos(localDinoData);

    		   });

}

function		createDinos(dinoData) 												{

				for (var i = dinoData["Dinos"].length - 1; i >= 0; i--) 	{

						console.log(dinoData["Dinos"][i]);

						let currentDino 										= dinoData["Dinos"][i];

						let newDino											= new Dino(currentDino);

						console.log("Dino ", newDino);
